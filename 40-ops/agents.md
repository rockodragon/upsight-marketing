...
- **Reads first:** `00-control/priorities.md`, `00-control/status.md`, `40-ops/agents.md` (this file)
- **Writes to:** `00-control/status.md` (end of session), build artifacts in repo, GTM file updates in vault
- **Owns:** code, sprint reports, drafting LinkedIn posts, updating pipeline.md, refreshing themes.md from MCP
...
- **Refreshes:** `30-research/themes.md`, `30-research/evidence-log.md` weekly via Claude Code
...
- Status: Tailscale auth issue parked, see `10-build/now.md`
...
> "Read `00-control/priorities.md` and `10-build/now.md`. What's the highest-priority item I should work on right now? Don't start work yet — confirm the plan first."

### Claude Code — end of session
> "Append a dated entry to `00-control/status.md` with: Shipped / Stuck / Decided / Surfaced. Then update `20-gtm/pipeline.md` if any deal moved today."

### Claude Code — weekly research refresh
> "Pull top themes from UpSight MCP for the current project. Update `30-research/themes.md` with mention counts, top 3 people per theme, and one paraphrased quote each. Date the refresh."

### Claude chat — strategy session
> "Here's `priorities.md`, `status.md`, and `pipeline.md`. [Paste contents.] Pressure-test my top 5. What am I missing or wrong about?"
...
This vault file (40-ops/agents.md) covers cross-domain agent operating rules. Code-specific things stay on the code side.
