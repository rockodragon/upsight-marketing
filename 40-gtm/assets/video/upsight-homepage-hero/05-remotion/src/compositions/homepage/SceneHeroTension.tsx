...
export const SceneHeroTension: React.FC = () => {
  ...
  const scene = homepageHeroScript.scenes[0];
  ...
  const accent = scene.subheadAccent ?? "";
  const base = (scene.subhead ?? "").replace(accent, "").trimEnd();

  return (
    <AbsoluteFill
      style={{ justifyContent: "center", alignItems: "center", opacity: sceneFade }}
    >
      <MonoLabel text={scene.label ?? "// the gap"} delay={4} />
      ...
      >
        ...
        >
          {base}{" "}
          <span style={{ color: colors.amber, fontWeight: 700 }}>{accent}</span>
        </div>
      </div>

      <div style={{ position: "absolute", inset: 0, opacity: fragmentFade }}>
        {homepageHeroScript.fragments.map((label, i) => {
          const pos = FRAGMENT_LAYOUT[i];
          ...
          return (
            <div
              ...
              style={{
                ...
                left: "50%",
                top: "50%",
                transform: `translate(${pos.x}px, ${pos.y + drift}px) rotate(${pos.rot}deg)`,
                opacity: enter * 0.55,
                padding: "10px 18px",
                borderRadius: 8,
