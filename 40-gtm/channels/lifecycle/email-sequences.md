...
## Smart Redirect (`/go/:destination`)

All email CTAs use `/go/` URLs that automatically resolve to the user's last-used project. **No user action or configuration needed** — it's fully automatic.

### How it works

1. User clicks CTA in email (e.g. `https://getupsight.com/go/evidence?utm_source=brevo`)
2. `/go` route checks if user is logged in
   - **Not logged in** → redirects to `/login?redirect=/go/evidence?...` → after login, lands on their project
   - **Logged in** → resolves `last_used_account_id` + `last_used_project_id` from `user_settings`
3. Redirects to `/a/{accountId}/{projectId}/evidence?utm_source=brevo`
4. Query params (UTM tags, discount codes) are preserved through the redirect

### CTA URL format for Brevo templates

```
https://getupsight.com/go/{destination}?utm_source=brevo&utm_medium=email&utm_campaign={sequence}
```

| Destination | Resolves to |
|-------------|-------------|
| `/go` | Project dashboard |
| `/go/evidence` | Evidence page |
| `/go/insights` | Insights page |
| `/go/surveys/new` | Create new survey |
| `/go/settings` | Project settings |
| `/go/team/invite` | Team invite page |
| `/go/billing/upgrade?code=EARLYBIRD25` | Billing with discount code |
| `/go/interviews/new` | Start new interview |
| `/go/people` | People directory |

### Edge cases

- **No projects** → redirects to `/home`
- **Invalid destination** → still redirects to project root + path (target route handles 404)
- **Multiple projects** → uses last-used project (same as login flow)
